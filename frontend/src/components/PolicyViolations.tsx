...
import { Container, Title, Paper, Table, Badge, Text, Stack, Group, Button } from '@mantine/core';
...
export function PolicyViolations() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await fetch('http://localhost:8000/api/v1/policies/violations/');
    ...
    const rows = violations.map((v) => (
        <tr key={v.id}>
            ...
            <td>
                {v.model_id ? (
                    <Text c="blue" style={{ cursor: 'pointer' }} onClick={() => navigate(`/models/${v.model_id}`)}>
                        Model #{v.model_id}
                    </Text>
                ) : '-'}
            </td>
