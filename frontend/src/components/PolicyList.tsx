...
import { Container, Title, Paper, Table, Badge, Button, Group, Modal, TextInput, Select, Textarea, Switch } from '@mantine/core';
...
export function PolicyList() {
    ...
    const handleCreatePolicy = async () => {
        try {
            ...
            if (res.ok) {
                ...
                close();
                setNewPolicy({ name: '', description: '', scope: 'global', condition_type: 'require_evaluation_before_approval', is_active: true });
            }
        } catch {
    ...
    const rows = policies.map((policy) => (
        <tr key={policy.id}>
            ...
            <td>{CONDITION_LABELS[policy.condition_type] || policy.condition_type}</td>
            <td>
        ...
        <Container size="lg" py="xl">
            ...
            <Modal opened={opened} onClose={close} title="Create New Policy">
                ...
                <Select
                    ...
                    onChange={(v) => setNewPolicy({ ...newPolicy, scope: v as PolicyScope })}
                    mb="md"
                />
                <Select
                    label="Condition Type"
                    data={[
                        { value: 'require_evaluation_before_approval', label: 'Require Evaluation Before Approval' },
                        { value: 'block_high_risk_without_approval', label: 'Block High Risk Without Review' },
                        { value: 'require_review_for_high_risk', label: 'Require Review for High Risk' }
                    ]}
                    value={newPolicy.condition_type}
                    onChange={(v) => setNewPolicy({ ...newPolicy, condition_type: v as PolicyConditionType })}
                    mb="md"
                />
                <Switch
                    label="Active"
                    checked={newPolicy.is_active}
                    onChange={(e) => setNewPolicy({ ...newPolicy, is_active: e.currentTarget.checked })}
                    mb="md"
                />
                <Button fullWidth onClick={handleCreatePolicy}>Create Policy</Button>
            </Modal>
        </Container>
    );
}
